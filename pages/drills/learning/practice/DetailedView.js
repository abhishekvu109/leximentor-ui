...
const Main = ({word, currentIndex, wordCount}) => {
    ...
    return (<>
        <div className="w-[55%]  p-2">
            <div className="flex flex-row justify-center">
                <div className="w-1/4">
                    <div className="flex flex-col">
                        <div className="basis-3/4">
                            ...
                            <CircularProgressBar
                                progressInfo={Math.round((currentIndex / wordCount) * 100)}></CircularProgressBar>
                        </div>
                        {/*Pronunciation speaker*/}
                        <div className="basis-1/4 ml-11 mt-3">
                            <div className="bg-blue-500 p-2 w-10 rounded-full shadow-md">
                                <Link href="#" onClick={() => handleConvertToSpeech(word.data.word)}><SpeakerWaveIcon
                ...
                <div className="w-1/2">
                    {/*<WordPanel word="Cerebral" pos="Adjective"></WordPanel>*/}
                    <WordFlashcard word={word} meanings={["Meaning-1", "Meaning-2"]}></WordFlashcard>
                </div>
            </div>
        </div>
        <div className="w-[55%] p-2 mt-2 justify-items-center">
            <div className="grid grid-cols-2 gap-x-10 gap-y-5">
                <SynonymFlashcard word={word.data.word} synonyms={word.data.synonyms}/>
                <AntonymFlashcard word={word.data.word} antonyms={word.data.antonyms}/>
                <MnemonicFlashcard word={word.data.word} mnemonic={word.data.mnemonic}/>
                <PronunciationFlashcard word={word.data.word} pronunciation={word.data.pronunciation}/>
            </div>
        </div>
        <div className="w-[55%] p-2">
            <div className="flex flex-row justify-center">
                <div className="w-3/4">
                    <CollapsibleWidget body={word.data.examples}/>
                </div>
            </div>
        </div>
    </>);
...
export const DetailedView = ({drillSetData, drillId, wordMetadata, sourcesData}) => {
    ...
    return (<>
        <div className="w-[55%] p-2 mb-2 justify-items-center">
            <div className="flex flex-row">
                <div><Sources allSources={sources} onClick={handleSources}/></div>
            </div>
        </div>
        <Main word={wordData} currentIndex={currentIndex + 1} wordCount={size}/>
        <div className="w-[55%] p-2 mt-2">
